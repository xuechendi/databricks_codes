...
from .exceptions import BadObjectiveFunction
    ...
    def __init__(
        ...
        if self.asynchronous:
            ...
            try:
                ...
                if "cannot pickle '_thread.RLock' object" in str(e):
                    raise BadObjectiveFunction(
                        "When using `fmin` asynchronously, distributed algorithms or "
                        "distributed objects may not be used within the objective function. "
                        "This includes algorithms from Apache Spark ML and data objects like "
                        "Spark DataFrames. "
                        "In order to use Apache Spark in the "
                        "objective function, use `Trials` instead of `SparkTrials`. "
                        "To instead use `fmin` for single-machine ML "
                        "like scikit-learn, make sure the objective function does not "
                        "reference a Spark DataFrame or a distributed algorithm. "
                        "See the following docs for more details on using Spark with Hyperopt: "
    ...
    def is_cancelled(self):
        ...
        if hasattr(self.trials, "_fmin_cancelled"):
            if self.trials._fmin_cancelled:
                logger.warning(
                    "fmin cancelled because of " + self.trials._fmin_cancelled_reason
                )
                return True
        return False
        ...
        with self.progress_callback(
            ...
            while (
                ...
                if losses:
                    ...
                    mlflow_log_best_loss = getattr(
                        self.trials, "mlflow_log_best_loss", None
                    )
                    if callable(mlflow_log_best_loss):
                        mlflow_log_best_loss(best_loss)
