...
class Forecast:
    ...
    def fit(self, **kwargs):
        if shared_utils.is_automl_service_enabled():
            return internal.forecast.Forecast(context_type=self._context_type).fit(**kwargs)
        else:
            return legacy.forecast.Forecast(context_type=self._context_type).fit(**kwargs)
