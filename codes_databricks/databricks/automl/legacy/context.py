...
from databricks.automl.legacy.clients.jobs import JobsClient
from databricks.automl.legacy.clients.workspace import WorkspaceClient
from databricks.automl.legacy.const import ContextType, DatasetFormat, MLFlowFlavor, RunState
from databricks.automl.legacy.errors import ExecutionTimeoutError, InvalidArgumentError, \
    ExperimentInitializationError
from databricks.automl.legacy.errors import NotebookImportSizeExceededError, TrialFailedError, \
    UnsupportedRuntimeError, UnsupportedDataError, ExperimentDirectoryDoesNotExist
from databricks.automl.legacy.plan import Plan
from databricks.automl.legacy.problem_type import Metric
from databricks.automl.legacy.result import TrialInfo
from databricks.automl.legacy.sections.training.databricks import DatabricksPredefined
from databricks.automl.legacy.sections.training.jupyter import JupyterDefinitions
from databricks.automl.legacy.tags import Tag
from databricks.automl.legacy.template import SectionTemplateManager
...
class DBUtilsInfo:
    ...
    def display_html(html) -> None:
        databricks_utils._get_dbutils().notebook.displayHTML(html)

    @staticmethod
    def create() -> DBUtilsInfo:
        ...
        driver_notebook_path = ctx.notebookPath().get() if ctx.notebookPath().isDefined() else None
    ...
    def run_notebook(path: str, timeout: Optional[int]) -> str:
        return databricks_utils._get_dbutils().notebook.run(path, timeout_seconds=timeout or 0)
...
class Context(ABC):
    ...
    def save_training_data(self, dataset: DataFrame, data_dir: Optional[str],
                           data_format: DatasetFormat,
                           selected_cols: List[str]) -> Tuple[DataSource, DataFrame]:
        """
    ...
    @abstractmethod
    def get_experiment_url(self, sort_metric: Optional[Metric], absolute: bool = False) -> str:
        """
    ...
    @abstractmethod
    def _execute_notebook_job(self,
                              plan: Plan,
                              save_dir: str,
                              timeout: Optional[int] = None,
                              max_retries: int = 3) -> Tuple[NotebookDetails, Dict[str, Any]]:
    ...
    def execute_data_exploration_job(
        ...
        :return: (Notebook URL, Result of data exploration)
        """
        details, result = self._execute_notebook_job(plan, self._session_root_dir, timeout,
                                                     max_retries)
        if not data_source.is_dbfs:
            self._set_mlflow_run_source(
                run_id=data_source.run_id, notebook_name=plan.name, notebook_id=details.id)
    ...
    def _execute_notebook_with_ipykernel(self,
        ...
        while True:
            try:
                ...
                # Don't retry this trial if we failed because of OOM or if we have exhaused our retries.
                # Some hyperparameters are more susceptible to using more memory and OOMing; so if a trial
                # with a given set of hyperparameters OOMs then it's better to not retry it and eat up more memory
                # and instead let hyperopt pick some other hyperparameters for the trial
                if isinstance(e, DeadKernelError) or num_retries > max_retries:
                    ...
                    _logger.debug(f"Retrying error: {repr(e)}")
                    num_retries += 1
...
class JupyterContext(Context):
    ...
    def get_experiment_url(self, sort_metric: Optional[Metric], absolute: bool = False) -> str:
        url = f"{mlflow.get_tracking_uri()}/{self.experiment_id}"
        if sort_metric:
            asc = json.dumps(not sort_metric.higher_is_better)
            url = f"{url}/s?orderByKey=metrics.%60{sort_metric.trial_metric_name}%60&orderByAsc={asc}"
        return url
...
class DatabricksContext(Context):
    ...
    def __init__(self, session_id: str, dbutils_info: DBUtilsInfo, experiment_dir: Optional[str]):
        self._dbutils_info = dbutils_info

        self._ws_client = WorkspaceClient(self._dbutils_info.api_url, self._dbutils_info.api_token)
        self._jobs_client = JobsClient(self._dbutils_info.api_url, self._dbutils_info.api_token)

        self._host_name = self._dbutils_info.host_name

        if self._dbutils_info.driver_notebook_path:
            self._driver_notebook_name = os.path.split(self._dbutils_info.driver_notebook_path)[1]
            self._driver_notebook_url = self._ws_client.get_notebook_url(
                self._dbutils_info.driver_notebook_path)
        else:
            self._driver_notebook_name = ""
            self._driver_notebook_url = ""

        experiment_dir = self._init_experiment_dir(experiment_dir, dbutils_info)
    ...
    def _init_experiment_dir(self, experiment_dir: Optional[str], dbutils_info: DBUtilsInfo) -> str:
        """
        ...
        :param dbutils_info:   DBUtils Context Info
        :return:               Validated user passed experiment_dir or default experiment_dir or throw
                               appropriate exception
        """
        default_experiment_dir = os.path.join("/Users", self._dbutils_info.user)
        ...
        if experiment_dir is not None:
            ...
            if dbutils_info.is_user_dbadmin and \
    ...
    def get_experiment_url(self, sort_metric: Optional[Metric], absolute: bool = False) -> str:
        url = f"#mlflow/experiments/{self.experiment_id}"
        if sort_metric:
            asc = json.dumps(not sort_metric.higher_is_better)
            url = f"{url}/s?orderByKey=metrics.%60{sort_metric.trial_metric_name}%60&orderByAsc={asc}"
        if absolute:
    ...
    def to_absolute_url(self, relative_url: str) -> str:
        if self._host_name:
            ...
            warnings.warn("No host name to create absolute URL")
            return relative_url

    def display_html(self, html) -> None:
        self._dbutils_info.display_html(html)
    ...
    def _pre_execution_setup(self, plan: Plan) -> Plan:
        plan = deepcopy(plan)
        predef_configs = DatabricksPredefined(self._dbutils_info.api_url,
                                              self._dbutils_info.api_token)
        plan.prepend(predef_configs)
        return plan
    ...
    def _execute_notebook_job(self,
        ...
        # run the notebook
        result = self._dbutils_info.run_notebook(details.path, timeout)
    ...
    def set_experiment_init(self,
        ...
        super().set_experiment_init(
            ...
            problem_type,
            evaluation_metric,
            job_run_id=self._dbutils_info.job_run_id)
...
class ContextFactory:
    ...
    def get_context(context_type: ContextType, session_id: str,
        ...
        if context_type == ContextType.JUPYTER:
            ...
            dbutils_info = DBUtilsInfo.create()
            return DatabricksContext(
                session_id=session_id, dbutils_info=dbutils_info, experiment_dir=experiment_dir)
