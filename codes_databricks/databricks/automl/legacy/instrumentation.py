...
from databricks.automl.legacy.errors import AutomlError
...
def log_forecast_data_stats(spark, notebook_id: int, num_target_nulls: int, num_rows: int,
                            num_cols: int, run_id: str) -> None:
    """
    This logs some stats about forecasting data
    :param spark: Active SparkSession
...
def log_supervised_input_data_stats(spark: SparkSession, stats: PreSamplingStats,
                                    run_id: str) -> None:
    pre_sampling_data_stats = InputDataStats(
        num_rows=stats.num_rows,
        num_cols=len(stats.columns),
        num_string_cols=len(stats.str_columns),
    ...
    _flush_event(spark=spark, event=pre_sampling_data_stats, run_id=run_id)


def log_supervised_data_stats(spark: SparkSession, notebook_id: int, num_target_nulls: int,
