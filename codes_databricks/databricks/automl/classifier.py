...
class Classifier:
    ...
    def fit(self, **kwargs):
        if shared_utils.is_automl_service_enabled():
            return internal.classifier.Classifier(context_type=self._context_type).fit(**kwargs)
        else:
            return legacy.classifier.Classifier(context_type=self._context_type).fit(**kwargs)
